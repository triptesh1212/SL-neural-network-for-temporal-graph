...
from utils.memory_module import MemoryLP, compute_src_dst_node_time_shifts
from utils.graph import NeighborFinder
from utils import EarlyStopMonitor2, NegativeEdgeSampler

parser = argparse.ArgumentParser('JODIE experiments on temporal link prediction')
parser.add_argument('-d', '--data', type=str, default='wikipedia')
parser.add_argument('--bs', type=int, default=200, help='batch_size')
parser.add_argument('--prefix', type=str, default='', help='prefix to name the checkpoints')
parser.add_argument('--n_degree', type=int, default=10, help='number of neighbors (unused by JODIE)')
parser.add_argument('--n_head', type=int, default=2, help='number of heads (unused by JODIE)')
parser.add_argument('--n_epoch', type=int, default=100, help='number of epochs')
parser.add_argument('--n_layer', type=int, default=1, help='number of layers (unused by JODIE)')
...
parser.add_argument('--time_dim', type=int, default=100, help='time feature dimension')
...
def eval_one_epoch(hint, model, sampler, src, dst, ts, e_idx, label):
    ...
    with torch.no_grad():
        ...
        for k in range(num_test_batch):
            ...
            e_idx_batch = min(num_test_instance, s_idx + test_batch_size)
            src_l_cut = src[s_idx:e_idx_batch]
            dst_l_cut = dst[s_idx:e_idx_batch]
            ts_l_cut = ts[s_idx:e_idx_batch]
            e_l_cut = e_idx[s_idx:e_idx_batch]
            size = len(src_l_cut)
            _, dst_l_fake = sampler.sample(size)
            pos_prob, neg_prob = model.contrast(
                src_l_cut, dst_l_cut, dst_l_fake, ts_l_cut, e_l_cut, NUM_NEIGHBORS)
...
src_mean, src_std, dst_mean, dst_std = compute_src_dst_node_time_shifts(
    train_src_l, train_dst_l, train_ts_l)
...
model = MemoryLP(
    ...
    num_heads=NUM_HEADS,
    drop_out=DROP_OUT,
    src_node_mean_time_shift=src_mean,
    src_node_std_time_shift=src_std,
    dst_node_mean_time_shift_dst=dst_mean,
...
for epoch in range(NUM_EPOCH):
    # memory models process edges in chronological order
    ...
    val_acc, val_ap, _, val_auc = eval_one_epoch(
        'val old', model, val_rand_sampler, val_src_l, val_dst_l, val_ts_l, val_e_idx_l, val_label_l)
    val_backup_memory_bank = model.memory_bank.backup_memory_bank()

    model.memory_bank.reload_memory_bank(train_backup_memory_bank)
    nn_val_acc, nn_val_ap, _, nn_val_auc = eval_one_epoch(
        'val new', model, nn_val_rand_sampler, nn_val_src_l, nn_val_dst_l, nn_val_ts_l, nn_val_e_idx_l, nn_val_label_l)

    # restore validation memory for checkpointing / later testing
...
model.ngh_finder = full_ngh_finder
val_backup_memory_bank = model.memory_bank.backup_memory_bank()
test_acc, test_ap, _, test_auc = eval_one_epoch(
    'test old', model, test_rand_sampler, test_src_l, test_dst_l, test_ts_l, test_e_idx_l, test_label_l)

model.memory_bank.reload_memory_bank(val_backup_memory_bank)
nn_test_acc, nn_test_ap, _, nn_test_auc = eval_one_epoch(
    'test new', model, nn_test_rand_sampler, nn_test_src_l, nn_test_dst_l, nn_test_ts_l, nn_test_e_idx_l, nn_test_label_l)
...
logger.info('JODIE model saved to {}'.format(MODEL_SAVE_PATH))
