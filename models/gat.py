...
from utils.gat_module import GATAN
from utils.graph import NeighborFinder
from utils import EarlyStopMonitor, RandEdgeSampler

parser = argparse.ArgumentParser('Interface for GAT experiments on link predictions')
...
MODEL_SAVE_PATH = f'./saved_models/{args.prefix}-gat-{args.data}.pth'
get_checkpoint_path = lambda epoch: f'./saved_checkpoints/{args.prefix}-gat-{args.data}-{epoch}.pth'
...
model = GATAN(
    ...
    num_layers=NUM_LAYER, n_head=NUM_HEADS, drop_out=DROP_OUT,
)
...
for epoch in range(NUM_EPOCH):
    ...
    val_acc, val_ap, val_f1, val_auc = eval_one_epoch(
        'val for old nodes', model, val_rand_sampler, val_src_l, val_dst_l, val_ts_l, val_label_l
    )
    nn_val_acc, nn_val_ap, nn_val_f1, nn_val_auc = eval_one_epoch(
        'val for new nodes', model, val_rand_sampler, nn_val_src_l, nn_val_dst_l, nn_val_ts_l, nn_val_label_l
    )
...
test_acc, test_ap, test_f1, test_auc = eval_one_epoch(
    'test for old nodes', model, test_rand_sampler, test_src_l, test_dst_l, test_ts_l, test_label_l
)
nn_test_acc, nn_test_ap, nn_test_f1, nn_test_auc = eval_one_epoch(
    'test for new nodes', model, nn_test_rand_sampler, nn_test_src_l, nn_test_dst_l, nn_test_ts_l, nn_test_label_l
)
